...
void print_stage_zero() {
    ...
    printf(NOSFMT, statementcounter);
    // Printing all the counted characters //
    printf(NOCFMT, charcounter);
    // Printing the number of statements accessed //
    printf(NPSFMT, statecount);
}
...
void generate_phase(state_t* latestaccessed, char* arrayrequired) {
    ...
    unsigned int cursorfreq = cursor->state->freq;
    ...
    while (cursor) {
        ...
        if ((cursorfreq) == max_freq || (cursorfreq+'a'-'A') == max_freq) {
