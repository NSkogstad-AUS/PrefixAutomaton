...
void replay_phase(automaton_t* automaton, const char* input) {
    ...
    for(int i = 0; i < input_length; i++) {
        ...
        while (cursor) {
            if (cursor->str[0] == input[i]) {
                arr[idx++] = input[i];
                latest_state = current_state->outputs->head->state;
...
void generate_phase(state_t* latestaccessed, char* arrayrequired) {
    if (!latestaccessed || !latestaccessed->outputs) { return; }
    
    node_t* cursor = latestaccessed->outputs->head;
    node_t* highest_freq_node = NULL;
    int highest_freq = 0;

    while (cursor) {
        if (cursor->state->freq > highest_freq) {
            highest_freq = cursor->state->freq;
            highest_freq_node = cursor;
        }
        cursor = cursor->next;
    }

    if (highest_freq_node) {
        int i = strlen(arrayrequired);
        arrayrequired = realloc(arrayrequired, (i + 2) * sizeof(char));
        arrayrequired[i] = *(highest_freq_node->str);
        arrayrequired[i+1] = '\0';

        generate_phase(highest_freq_node->state, arrayrequired);
    } else {
        printf("new: | %s\n", arrayrequired);
    }
}
