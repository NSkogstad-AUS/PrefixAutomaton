...
void splitInteger(int newcomp, int *splitValues, int *digit_count);
int commaSep(int n);
...
void parse_num(char *rhs, longint_t *secondvalArr) {
	...
	/* Putting it into another array that is offset by 1 to accomodate for 
    digit count*/
...
do_plus(longint_t *var1, longint_t *var2) {
    ...
    /* Given var2 has more digits, fill var1 with zeros to avoid weird 
    addition */
    if (intdiff > 0) {
	...
	for (int i = 1; i < INTSIZE+1; i++) {
	    /*checking if var1 and var2 have valid digits (so we don't use -1 
        values) */
	    if (var1[i] >= 0 && var2[i] >= 0) {
	        ...
	        if (var1[i] + var2[i] >= 10) {
                ...
                /* We need to split the integer because we can't have a two dig
                it num occupying a single digit in an array */
